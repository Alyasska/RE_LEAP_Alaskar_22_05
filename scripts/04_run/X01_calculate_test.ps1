# X01_calculate_test.ps1
# Trigger Calculate on a given area index, capture results and errors.
# This is the test-loop script we use repeatedly Days 3-6 to verify fixes.
#
# Usage:
#   .\scripts\04_run\X01_calculate_test.ps1 -AreaIndex 6
#   .\scripts\04_run\X01_calculate_test.ps1 -AreaIndex 6 -OnlyScenario "S0"
#
# What it does:
#   1. Open area by index
#   2. Verify open (BaseYear sanity)
#   3. Optionally filter to one scenario
#   4. Call oLEAP.Calculate (or whatever the COM method is)
#   5. Capture timing, success/error, error text if any
#   6. Read LEAP's diagnostics if available
#   7. Write a markdown report
#
# Note: this script does NOT save changes. If LEAP prompts on close, click No.
#       If the script accidentally enables a "save" behavior, please tell Claude.
...
$dataFile   = Join-Path $projectRoot "data\audit_reports\calctest_idx${AreaIndex}_${timestamp}.data.txt"
$reportPath = Join-Path $projectRoot "data\audit_reports\calctest_idx${AreaIndex}_${timestamp}.md"
$runLogPath = Join-Path $projectRoot "logs\X01_idx${AreaIndex}_${timestamp}.log"
...
end_time = Timer
W "CALC_ELAPSED_SECONDS", FormatNumber(end_time - start_time, 2)
...
$vbs = $vbsTemplate.Replace('{{AREA_INDEX}}', "$AreaIndex").Replace('{{DATA_FILE}}', $dataFile.Replace('\','\\')).Replace('{{ONLY_SCENARIO}}', $OnlyScenario)
